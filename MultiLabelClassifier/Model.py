...
class MultiLabelClassifier_Swin_L_Window12_192(LightningModule):
    def __init__(
            self,
            input_shape: Tuple[int, int] = (192, 192),  # must be 192*192 as we use pretrained Swinv2_L_window12_192
            num_heads: int = 8,  # heads number in [1, 2, 4, 6, 8]
            attention_lambda: float = 0.3,
            num_classes: int = 6,
            thresh: float = 0.5,
        ...
        # networks
        self.backbone = Swinv2_L_Window12_192_Extractor(True)
        self.classify_head = ClassSpecificMultiHeadAttention(num_heads, attention_lambda, 1024, num_classes)  # embed dim=1024
        self.example_input_array = [torch.zeros(batch_size, 3, input_shape[0], input_shape[1])]
...
class MultiLabelClassifier_ViT_L_Patch14_336(LightningModule):
    def __init__(
            self,
            input_shape: Tuple[int, int] = (336, 336),  # must be 336*336 as we use pretrained ViT_L_Patch14_336
            num_heads: int = 8,  # heads number in [1, 2, 4, 6, 8]
            attention_lambda: float = 0.3,
            num_classes: int = 6,
            thresh: float = 0.5,
